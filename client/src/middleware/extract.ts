...
import {gobleQuery} from "../config/ConfigManager";
...
const ExtractMiddleware = async (intlConfigTemp: string, workspaceIntlConfigPath: Uri | undefined, args: any[], next: ExecuteCommandSignature) =>
{
	...
	if (intlId)
	{
		gobleQuery.query=intlId.split('.')[1]
	...
	const { newArgs,
		// customIntlId
	} = await VSCodeUIManager.getInstance().processArgsWithSelectResult(args, selectedIntlId, specialStringParams)
	...
	try
	{
		...
		await IntlConfigManager.getInstance().writeConfigIntoWorkSpace(
			// `page.${(customIntlId || selectedIntlId)}${gobleQuery.query}`
			`page.${gobleQuery.query||selectedIntlId}`,
